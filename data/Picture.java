...
public final class Picture implements ActionListener {
    ...
    public Picture(BufferedImage image) {
        ...
        if (image == null) {
            throw new IllegalArgumentException("could not read image: " + "buffered");
        }
        this.image = image;
        width = image.getWidth(null);
        height = image.getHeight(null);
    ...
    /**
     * Returns a byte array containing the bicture info
     */
    public byte[] getByte() {
        int w = width;
        int h = height;
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                color[i][j] = get(j, i).getRGB();
            }
        }

        byte[] results = new byte[width() * height() * 3];
        ...
        for (int i = 0; i < height(); i++) {
            for (int j = 0; j < width(); j++) {
                byte r = (byte) (((color[i][j] & mask_r) >> 16) & 0xff);
                byte g = (byte) (((color[i][j] & mask_g) >> 8) & 0xff);
                byte b = (byte) (((color[i][j] & mask_b)) & 0xff);

                results[j + i * w] = r;
                results[j + i * w + h * w] = g;
                results[j + i * w + h * w * 2] = b;
            }
        }
        return results;
    }
